...
def run_module():
    ...
    if not module.check_mode:
        try:
            client.post('/dedicated/nasha/{0}/partition'.format(nas_service_name), size=nas_partition_size, partitionName=nas_partition_name, protocol=nas_protocol)
        except APIError as api_error:
            module.fail_json(msg="Failed to create partition: %s" % e)           
    else:
        try:
            client.get('/dedicated/nasha/{0}'.format(nas_service_name))
        except APIError as api_error:
            module.fail_json(msg="Failed to get partition: %s" % e)

    ## Set partition ACL
    if not module.check_mode:
        for ip_acl in nas_partition_acl:    
            try:
                client.post('/dedicated/nasha/{0}/partition/{1}/access'.format(nas_service_name, nas_partition_name), ip=ip_acl, type=nas_partition_acl_type)
                module.exit_json(msg="IP {} has been added to ACL of {} partition".format(ip_acl, nas_partition_name), changed=True)
            except APIError as api_error:
                module.fail_json(msg="Failed to set partition ACL: %s" % e)
